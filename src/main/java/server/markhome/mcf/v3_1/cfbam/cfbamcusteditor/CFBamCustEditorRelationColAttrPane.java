 ...
 *	for RelationCol.
 */
public class CFBamCustEditorRelationColAttrPane
extends CFGridPane
implements ICFBamJavaFXRelationColPaneCommon
{
				...
				if( cur != null ) {
					...
					if( null != editObj ) {
						...
						if( ( curMode == CFPane.PaneMode.Add ) || ( curMode == CFPane.PaneMode.Edit ) ) {
							...
							javafxEditorName.setStringValue( value.getObjName() );
							javafxEditorShortName.setStringValue( value.getOptionalShortName() );
							javafxEditorDescription.setStringValue( value.getOptionalDescription() );
							javafxEditorShortDescription.setStringValue( value.getOptionalShortDescription() );
							javafxEditorLabel.setStringValue( value.getOptionalLabel() );
						}
					}
				}
			}
	...
	protected class CompareIndexColByName
	implements Comparator<ICFBamIndexColObj>
	{
		public CompareIndexColByName() {
		}

		@Override
		public int compare(ICFBamIndexColObj o1, ICFBamIndexColObj o2) {
			if( o1 == null ) {
				if( o2 == null ) {
					return( 0 );
				}
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			List<ICFBamIndexColObj> listOfIndexCol = null;
			...
			ICFBamIndexColObj[] arrIndexCol = new ICFBamIndexColObj[listOfIndexCol.size()];
			Iterator<ICFBamIndexColObj> iterIndexCol = listOfIndexCol.iterator();
			int idx = 0;
			while( iterIndexCol.hasNext() ) {
				arrIndexCol[idx++] = iterIndexCol.next();
			}
			Arrays.sort( arrIndexCol, new CompareIndexColByName() );
			Collection<ICFBamIndexColObj> cltn = new LinkedList<ICFBamIndexColObj>();
			for( idx = 0; idx < arrIndexCol.length; idx++ ) {
				cltn.add( arrIndexCol[idx] );
			}
	...
	public CFBamCustEditorRelationColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamRelationColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public void postFields()
	{
		...
		if( editObj.getRequiredLookupFromCol() == null ) {
			throw new CFLibNullArgumentException( getClass(),
				S_ProcName,
				0,
				"FromCol" );
		...
		if( editObj.getRequiredLookupToCol() == null ) {
			throw new CFLibNullArgumentException( getClass(),
				S_ProcName,
				0,
				"ToCol" );
	...
	public void adjustComponentEnableStates() {
		...
		if( javafxEditorShortName != null ) {
			javafxEditorShortName.setDisable( true );
		}
		if( javafxEditorLabel != null ) {
			javafxEditorLabel.setDisable( true );
		}
		if( javafxEditorShortDescription != null ) {
			javafxEditorShortDescription.setDisable( true );
		}
		if( javafxEditorDescription != null ) {
			javafxEditorDescription.setDisable( true );
		}
	}
}
